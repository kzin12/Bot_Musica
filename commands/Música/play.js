...
module.exports = {
  ...
  if (!query) return error("Você não forneceu um nome de Um música para tocar!");
  ...
  if (query.includes("www.youtube.com")) {
    ...
    try {
      ...
      if (fetched.length === 0 || !fetched)
        return error("Não consegui encontrar a música que você solicitou!");
    ...
    try {
      ...
      data.channel.send(
        new MessageEmbed()
          ...
          .addField("Nome Da Musica ", track.name, false)
